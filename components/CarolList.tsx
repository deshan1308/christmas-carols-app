...
export default function CarolList() {
  const { carols, tempSelectedCarols, toggleTempCarolSelection, fetchCarols, isLoading } = useCarols()
  ...
  const handleToggleCarol = (carolId: number) => {
    ...
    if (carol && !carol.selected) {
      ...
      if (!isCurrentlySelected && tempSelectedCarols.length >= 2) {
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-xl font-bold text-gray-800">
            Select Carols ({tempSelectedCarols.length}/2 selected)
          </h3>
          {carols.length === 0 && (
            <button
              onClick={() => fetchCarols()}
